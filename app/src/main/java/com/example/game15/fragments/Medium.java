...
public class Medium extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        RecyclerView recycler = view.findViewById(R.id.recycler_medium);
        MyDatabase database = MyDatabase.getInstance(requireContext());
        List<Model> list = database.modelDao().getResultsByLevel(5);
        ...
        if (list.size() > 20) {
            ...
            list = database.modelDao().getResultsByLevel(5);
            Collections.sort(list, (o1, o2) -> Long.compare(o1.result, o2.result));
        }
        if (list.size() == 0) {
